...
class ModdingGUI:
    ...
    def setup_ui(self):
        ...
        # Новая кнопка проверки одного файла
        self.btn_check_one = self._create_styled_button(row3, self.run_check_one, text=self.get_str('check_one'))
        ...
        # Новая кнопка пакетной проверки
        self.btn_check_all = self._create_styled_button(row4, self.run_check_all, text=self.get_str('check_all'))
    ...
    def update_all_texts(self):
        ...
        # Обновляем тексты новых кнопок
        self.btn_check_one.config(text=self.get_str('check_one'))
        self.btn_check_all.config(text=self.get_str('check_all'))
    ...
    def disable_buttons(self, disabled):
        ...
        for widget in [self.btn_unpack, self.btn_decompile_all, self.btn_compile_all,
                       ...
                       self.btn_check_one, self.btn_check_all]:  # добавили новые кнопки
    ...
    # ==========================================================
    # ====   Проверка кода через Luacheck                 ====
    # ==========================================================

    def _run_luacheck(self, file_path):
        """Запускает luacheck.exe на указанном файле и возвращает вывод."""
        luacheck_exe = os.path.join(self.tools_dir.get(), 'luacheck.exe')
        if not os.path.exists(luacheck_exe):
            self.log(self.get_str('check_no_luacheck'))
            return self.get_str('check_no_luacheck')
        
        # Формируем команду: игнорируем глобальный конфиг, показываем коды ошибок
    ...
    def _check_file_thread(self, file_path):
        """Проверка одного файла в потоке (вывод в лог)."""
        self.log(self.get_str('check_single_start', os.path.basename(file_path)))
        output = self._run_luacheck(file_path)
        self.log(output)
        # Можно также показать в отдельном окне, но пока просто логируем

    def run_check_one(self):
        """Выбор одного .lua файла и его проверка."""
        file_path = filedialog.askopenfilename(
            title=self.get_str('check_one'),
    ...
    def _check_all_thread(self):
        """Пакетная проверка всех .lua из mod_scripts."""
        self.log(self.get_str('check_all_start'))
        mod_scripts = os.path.join(self.project_dir.get(), 'mod_scripts')
        if not os.path.exists(mod_scripts):
            self.log(self.get_str('file_manager_no_mod_scripts'))
            return
        ...
        for idx, fpath in enumerate(lua_files):
            ...
            # Можно также выводить краткий отчёт, но пока оставим так
    ...
    def run_check_all(self):
        """Запуск пакетной проверки в потоке."""
        if not self.project_dir.get():
            messagebox.showerror("Error", self.get_str('err_no_project'))
            return
        # Проверяем наличие luacheck.exe
        luacheck_exe = os.path.join(self.tools_dir.get(), 'luacheck.exe')
        if not os.path.exists(luacheck_exe):
    ...
    def run_check_one_from_manager(self, file_path):
        """Запускает проверку одного файла из менеджера."""
        self.run_in_thread(lambda: self._check_file_thread(file_path))
    ...
    # ===== Компиляция отдельного файла (используется в менеджере) =====
    def compile_single_file(self, file_path):
        """Компилирует один .lua файл (для кнопки 'Собрать' в менеджере)."""
        self.run_in_thread(lambda: self._compile_one_thread(file_path))
